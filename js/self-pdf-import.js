(() => {
 ...
 const fingerprint=()=>JSON.stringify({classes:allClasses,settings:scheduleSettings,weekly:weeklySchedule});
 ...
 function persist(){localStorage.setItem('viescoHub_allClasses',JSON.stringify(allClasses));localStorage.setItem('viescoHub_selfSettings',JSON.stringify(scheduleSettings));if(weeklySchedule)localStorage.setItem('viescoHub_selfWeekly',JSON.stringify(weeklySchedule));else localStorage.removeItem('viescoHub_selfWeekly');clearManualPlanning();autoGenerate();renderSettingsTable();shareSelfSettings();}
 $('pdf-undo').onclick=()=>{const raw=localStorage.getItem('viescoHub_backup');if(!raw){status('Aucun import à annuler.');return;}if(!confirm('Restaurer la configuration précédant le dernier import ? La version restaurée sera aussi partagée.'))return;const b=JSON.parse(raw);allClasses=b.classes;scheduleSettings=b.settings;weeklySchedule=b.weekly||null;persist();manualPlanningByContext=b.manual;localStorage.setItem(manualPlanningStorageKey,JSON.stringify(b.manual));updateDayHeader();$('pdf-preview').hidden=true;status('Configuration précédant le dernier import restaurée.');};
 $('pdf-file').onchange=async e=>{
  ...
  try {
   ...
   rows.forEach(r=>{const tr=elt('tr'),check=elt('input');tr.dataset.parent=r.parent;check.type='checkbox';check.disabled=r.day==='MERCREDI';r.check=check;let td=elt('td');td.append(check);tr.append(td);
    ...
    const old=elt('td');const refresh=()=>{const samePeriod=weeklySchedule?.start===r.periodStart&&weeklySchedule?.end===r.periodEnd;const s=(samePeriod?weeklySchedule?.settings?.[name.value]?.[r.day]:null)||scheduleSettings[name.value]?.[r.day]?.[r.week];old.textContent=s?`${s.fin||'—'} / ${s.reprise||'—'}`:'Non configuré';};name.onchange=refresh;refresh();tr.append(old);
 ...
 $('pdf-apply').onclick=()=>{
  ...
  const period=chosen[0].periodStart&&chosen.every(r=>r.periodStart===chosen[0].periodStart&&r.periodEnd===chosen[0].periodEnd)?{start:chosen[0].periodStart,end:chosen[0].periodEnd,weekType:chosen[0].week}:null;
  const action=period?`Remplacer la configuration hebdomadaire par celle du ${period.start.split('-').reverse().join('/')} au ${period.end.split('-').reverse().join('/')} (${chosen.length} lignes) ? Les horaires permanents A/B resteront intacts.`:`Appliquer ${chosen.length} lignes aux horaires permanents A/B ?`;
  if(!confirm(action+' Les placements manuels seront recalculés et la configuration sera partagée.'))return;
  localStorage.setItem('viescoHub_backup',JSON.stringify({classes:allClasses,settings:scheduleSettings,weekly:weeklySchedule,manual:manualPlanningByContext}));
  if(period){
   weeklySchedule={...period,settings:{},importedAt:new Date().toISOString()};
   for(const r of chosen){const name=r.nameInput.value;if(!allClasses.some(c=>c.name===name))allClasses.push({name,lvl:name[0]});weeklySchedule.settings[name]??={};weeklySchedule.settings[name][r.day]={fin:r.finInput.value.replace(':','h'),reprise:r.repInput.value.replace(':','h')};}
  } else {
  ...
  persist();$('pdf-preview').hidden=true;status(period?`${chosen.length} lignes enregistrées pour la semaine du ${period.start.split('-').reverse().join('/')} au ${period.end.split('-').reverse().join('/')}. Elles remplacent l’import hebdomadaire précédent.`:`${chosen.length} lignes permanentes enregistrées. Tu peux annuler le dernier import.`);
 };
 ...
 function classify(r){
  const samePeriod=weeklySchedule?.start===r.periodStart&&weeklySchedule?.end===r.periodEnd;
  const old=(samePeriod?weeklySchedule?.settings?.[r.nameInput.value]?.[r.day]:null)||scheduleSettings[r.nameInput.value]?.[r.day]?.[r.week];
 ...
 function filterResults(){
  ...
  const period=rows[0]?.periodStart?`Semaine du ${rows[0].periodStart.split('-').reverse().join('/')} au ${rows[0].periodEnd.split('-').reverse().join('/')} · `:'';
  overview.textContent=`${period}${counts.changed} horaires différents · ${counts.missing} configurations manquantes · ${counts.uncertain} lectures incertaines · ${counts.same} horaires identiques masqués · ${counts.excluded} lignes du mercredi exclues.`;
